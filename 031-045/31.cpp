...
vector<int>primeFactoize(int n){
    vector<int>temp;
    int x = n;
    for(int i = 2 ; i <= 100 ; i++){
        if(x % i == 0){
...
void solve(){
    vector<int>temp = {1 ,2 , 5, 10 , 20 , 50 , 100 , 200};
    int n = temp.size();
    vector<vector<int>>dp(9 , vector<int>(201 , -1));
    
    function<int(int , int)> f = [&](int curr , int rem) -> int{
        if(rem == 0 && curr == n) return 1;
        if(curr == n) return 0;
        ...
        //take
        if(temp[curr] <= rem){
            res = res + f(curr , rem - temp[curr]);
        }
        
        //no take
        res = res + f(curr + 1 , rem);
        
        return dp[curr][rem] = res;
    };
